...
const OPS = new Set(["submit", "poll", "cancel", "retry"]);
...
function parseReferenceBody(value: unknown): MediaGenRuntimeReference | null {
  ...
  if (kind === "artifact") {
    ...
    if (!artifactId || value.runtimeLocalRef !== undefined) {
      return null;
    }
    return {
      kind,
...
function parseReferenceSlot(value: unknown): MediaGenReferenceSlot | null {
  ...
  if (value.role !== undefined) {
    if (typeof value.role !== "string" || !REFERENCE_ROLES.has(value.role as MediaGenReferenceRole)) {
      return null;
    }
...
export function parseDispatch(value: unknown): MediaGenRuntimeDispatch | null {
  ...
  const consentRef =
    value.consentRef === undefined
      ? undefined
      : (asNonEmptyString(value.consentRef) ?? undefined);
  ...
  return {
    ...
    runtimeJobId: asNonEmptyString(value.runtimeJobId) ?? undefined,
  };
}
