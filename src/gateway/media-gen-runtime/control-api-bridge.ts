...
export function createControlApiMediaGenBridge(
  ...
  return {
    ...
    async resolveArtifactReference(input): Promise<MediaGenRuntimeSource> {
    ...
    async handoffArtifact(input): Promise<MediaGenRuntimeArtifactRef> {
      const res = await fetchImpl(
        buildHandoffUrl(baseUrl, options.runtimeId, input.dispatch, input.output, input.sha256),
        {
          method: "POST",
          headers: {
