...
export type MediaQualityValidatorDeps = {
  runFfprobe?: (args: string[], options?: MediaExecOptions) => Promise<string>;
  runFfmpeg?: (args: string[], options?: MediaExecOptions) => Promise<FfmpegCaptureResult>;
...
export function createMediaQualityValidator(
  ...
  return async (input) => {
    ...
    try {
      ...
      const execOpts: MediaExecOptions = {
        timeoutMs,
